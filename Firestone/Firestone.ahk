    ...
    BackToMainScreen(){
        ...
        loop 8 {
            ...
            if this.Buttons.Green.Wait(1032, 706, 1059, 780, 500) {
                ...
                DebugLog.Log('Проверяем не появлось ли новое окно `"Вам нравится игра?`"')
                if this.Buttons.Green.Check(1137, 588, 1155, 644) {
                    DebugLog.Log('Окно обнаружено')
                    ; Хорошо, мы на главном экране, можно продолжать скрипт
                    this.Click(1333, 292, 500) ;; Клик по окошку "Нравится игра?"
                }
                else
                {
                    DebugLog.Log('Окно не обнаружено')
