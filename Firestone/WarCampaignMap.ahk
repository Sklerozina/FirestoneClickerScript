Class WarCampaignMap {
    __New(Firestone) {
        ...
        this.missions := Map(
            'mystery', MapMissions(this.Firestone, [MapMission(954, 503), MapMission(728, 350)], Button(this.Firestone, 0xCF6639, 5)),
            ; ~20 minutes
            ...
            ], Button(this.Firestone, 0xFF6F11, 5)),
            ; ~40 minutes
            'adventure', MapMissions(this.Firestone, [MapMission(564, 262), MapMission(1289, 280), MapMission(1480, 227), MapMission(789, 476), MapMission(672, 569), MapMission(1433, 663),
                ...
                MapMission(1012, 500), MapMission(1418, 394), MapMission(1318, 517), MapMission(1411, 845)], Button(this.Firestone, 0xFFEE51, 5)),
            ; ~1-2 hours
            ...
            'sea', MapMissions(this.Firestone, [MapMission(1137, 312), MapMission(374, 953), MapMission(1245, 819), MapMission(836, 944, true)], Button(this.Firestone, 0x68E5F7, 5)), ; 836, 944 - эту только принудительно кликать
    ...
    CheckMapPosition() {
        if Tools.PixelSearch(406, 132, 408, 134, 0xB5C3CB, 1)
            return true
        else
            return false
    }
    ...
    CheckMissions(force := false){
        ...
        if (this.CheckSquad() || force == true)
        {
            ...
            this.missions.Get('mystery').EachMapMissions(force, false, this.Firestone.Settings.Get('force_click_map_missions', 0))

            for mission_type in this.Get_Priority()
            {
                DebugLog.Log('=== Прокликиваем ' mission_type ' ===')
                this.missions.Get(mission_type).EachMapMissions(force, false, this.Firestone.Settings.Get('force_click_map_missions', 0))
            }
        }
    }
